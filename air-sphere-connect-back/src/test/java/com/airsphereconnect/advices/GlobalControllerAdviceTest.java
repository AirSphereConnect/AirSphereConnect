...
class GlobalControllerAdviceTest {
    ...
    @Test
    @DisplayName("handleGlobalException should return 500 with ProblemDetail")
    void handleGlobalException_shouldReturn500() {
        GlobalException ex = new GlobalException.ResourceNotFoundException("Erreur applicative critique");

        ResponseEntity<ProblemDetail> response = advice.handleGlobalException(ex, request);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.INTERNAL_SERVER_ERROR);
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody().getTitle()).isEqualTo("Erreur applicative");
        assertThat(response.getBody().getDetail()).isEqualTo("Erreur applicative critique");
        assertThat(response.getBody().getInstance()).hasToString("/api/test");
    }
}
