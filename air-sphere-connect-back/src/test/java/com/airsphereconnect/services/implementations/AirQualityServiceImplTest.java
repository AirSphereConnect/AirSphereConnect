...
class AirQualityServiceImplTest {
    ...
    class GetCompleteDataTests {
        ...
        @Test
        @DisplayName("should handle NumberFormatException on invalid areaCode gracefully")
        void shouldHandleNumberFormatExceptionGracefully() {
            when(cityRepository.findByNameIgnoreCase("Montpellier")).thenReturn(Optional.of(city));
            when(indexRepository.findFirstByAreaCodeOrderByMeasuredAtDesc("243400017"))
                    .thenThrow(new NumberFormatException("Invalid EPCI code"));
            ...
            assertNotNull(result);
            assertNull(result.getQualityIndex());
        }
