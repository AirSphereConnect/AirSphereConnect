...
public interface AirQualityMeasurementRepository extends JpaRepository<AirQualityMeasurement, Long> {
    ...
    /**
     * Récupère les dernières mesures distinctes pour chaque station d'un département. Utilisé comme troisième niveau de la stratégie de fallback (recherche au niveau départemental).
     * Extrait le code département (2 premiers chiffres du code INSEE) et retourne uniquement la mesure la plus récente de chaque station.
     *
    ...
    /**
     * Récupère l'historique complet des mesures pour un département. Extrait le code département des 2 premiers chiffres du code INSEE.
     * Les résultats sont triés par date de mesure décroissante.
    ...
    /**
     * Récupère l'historique des mesures pour un département sur une période donnée. Filtre les mesures par département (2 premiers chiffres du code INSEE) et période temporelle.
     * @param departmentCode Le code du département (2 chiffres)
     * @param start          La date et heure de début de la période
     * @param end            La date et heure de fin de la période
     * @return La liste des mesures dans la période spécifiée, triée par date décroissante
