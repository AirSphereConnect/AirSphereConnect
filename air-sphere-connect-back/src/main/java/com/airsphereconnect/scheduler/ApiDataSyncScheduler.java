...
public class ApiDataSyncScheduler {
            ...
            syncServices.stream()
                    ...
                    .ifPresent(service -> {
                        ...
                        try {
                            ...
                            throw new GlobalException.SyncException("Échec critique de la synchronisation de " + serviceName, e);
                        }
                    });
        }
    ...
    public void annualRecensementSync() {
        ...
        syncServices.stream()
                ...
                .findFirst()
                .ifPresent(service -> {
                    log.info("🔄 Sync annuelle: {}", service.getServiceName());
                    syncService(service);
                });
    ...
    private void syncService(DataSyncService service) {
        ...
        try {
            ...
            throw new GlobalException.SyncException("Sync failed for " + service.getServiceName(), e);
        }
    }
