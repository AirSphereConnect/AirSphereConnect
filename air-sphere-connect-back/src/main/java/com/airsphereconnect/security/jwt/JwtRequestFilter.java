...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        log.debug("JWT Filter → Path: {} | Method: {}", requestPath, method);
        //Solution temporaire pour éviter les boucles infinies
        // 1. Skip endpoints publics (.permitAll()) et CORS preflight
        if (shouldSkipFilter(requestPath, method)) {
            log.debug("Skip JWT filter pour endpoint public: {}", requestPath);
        ...
        if (jwt == null || jwt.trim().isEmpty()) {
            log.debug("Pas de JWT → requête anonyme");
            chain.doFilter(request, response);
            return;
        }
        ...
        try {
            ...
            if (authenticateValidToken(request, jwt)) {
                log.debug("JWT valide → utilisateur authentifié");
            }
        } catch (Exception e) {
            log.warn("Erreur validation JWT '{}': {}", requestPath, e.getMessage());
