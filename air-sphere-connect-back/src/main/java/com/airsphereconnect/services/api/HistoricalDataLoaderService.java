...
public class HistoricalDataLoaderService {
    ...
    /**
     * 🔍 Détecte les jours sans données dans les 30 derniers jours et les comble.
     * Utilisé par le scheduler nocturne pour rattraper les données manquantes
     * si le serveur était éteint.
     *
     * @return le nombre de jours manquants détectés
     */
    public int detectAndFillMissingDays() {
        log.info("🔍 [HISTORIQUE] Détection des jours manquants...");

        LocalDate today = LocalDate.now();
        LocalDate thirtyDaysAgo = today.minusDays(30);

        // Récupérer les mesures des 30 derniers jours en base
        List<AirQualityMeasurement> recentMeasurements = measurementRepository
                .findByMeasuredAtAfterOrderByStation_IdAscMeasuredAtDesc(thirtyDaysAgo.atStartOfDay());

        // Extraire les jours distincts couverts
        Set<LocalDate> coveredDays = recentMeasurements.stream()
                .map(m -> m.getMeasuredAt().toLocalDate())
                .collect(Collectors.toSet());

        // Calculer les jours manquants (on exclut aujourd'hui, données pas encore publiées)
        List<LocalDate> missingDays = new ArrayList<>();
        LocalDate cursor = thirtyDaysAgo;
        while (!cursor.isAfter(today.minusDays(1))) {
            if (!coveredDays.contains(cursor)) {
                missingDays.add(cursor);
            }
            cursor = cursor.plusDays(1);
        }

        int missingCount = missingDays.size();

        if (missingCount == 0) {
            log.info("✅ [HISTORIQUE] Aucun jour manquant détecté");
            return 0;
        }

        log.info("⚠️ [HISTORIQUE] {} jour(s) manquant(s) détecté(s), lancement du rechargement...", missingCount);
        loadLast30DaysHistory();
        log.info("✅ [HISTORIQUE] Rechargement terminé pour {} jour(s) manquant(s)", missingCount);
